...
def combine_mc_components(
        tbl_n_component, tbl_component_process,
        tbl_nevt, tbl_xsec,
        lumi = 1,
        dataset_column = 'component',
        nevt_column = 'nevt'):

    tbl_out = combine_mc_yields_in_datasets_into_xsec_in_processes(
        tbl_n_component, tbl_component_process, tbl_nevt, tbl_xsec,
        dataset_column = dataset_column,
        nevt_column = nevt_column
    )

    tbl_out['n'] = tbl_out.xsec*lumi
    tbl_out['nvar'] = tbl_out.xsecvar*lumi**2
    tbl_out['luminosity'] = lumi
    del tbl_out['xsec']
    del tbl_out['xsecvar']

    sortby = [c for c in tbl_out.columns if c not in ('n', 'nvar', 'luminosity')]
    tbl_out = tbl_out.sort_values(by = sortby)
